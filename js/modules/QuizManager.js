...
export class QuizManager {
  ...
  /**
   * Tạo các template quiz mẫu
   */
  createQuizTemplates() {
    return [
      // Template 1: Giới thiệu từ vựng
      {
        type: "intro",
        html: `
      ...
      // Template 2: Câu hỏi trắc nghiệm
      {
   ...
   * @param {Object} wordData - Dữ liệu từ vựng
   */
  getAudioToViHTML(quizId, wordData) {
    // Lấy danh sách nghĩa sai ngẫu nhiên (khác nghĩa đúng)
    const wrongChoices = this.currentLessonWords
      .filter((w) => w.meaning !== wordData.meaning)
      .sort(() => Math.random() - 0.5) // shuffle
  ...
  getQuizHTMLByType(quizId, wordId, type) {
    switch (type) {
      case this.QUIZ_TYPES.INTRO:
        return this.getIntroHTML(quizId, this.getWordById(wordId));
      case this.QUIZ_TYPES.ENTOVI:
        return this.getEnToViHTML(quizId, this.getWordById(wordId));
      case this.QUIZ_TYPES.VITOEN:
        return this.getViToEnHTML(quizId, this.getWordById(wordId));
      case this.QUIZ_TYPES.AUDIO:
        return this.getAudioToViHTML(quizId, this.getWordById(wordId));
      case this.QUIZ_TYPES.SPELL:
        return this.getSpellHTML(quizId, this.getWordById(wordId));
    }
  }
