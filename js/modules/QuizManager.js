...
export class QuizManager {
  ...
  getIntroHTML(wordData) {
    ...
    if (exampleLength > 2) {
      for (let i = 0; i < exampleLength; i++) {
        examplesHtml += `
            <div class="vocab-example">
              <p class="example">${wordData.details.examples[i].en}</p>
            </div>
      `;
      }
    } else if (exampleLength === 1) {
      examplesHtml += `
            <div class="vocab-example">
              <p class="example">${wordData.details.examples[0].en}</p>
            </div>
      `;
    } else {
      examplesHtml = "";
  ...
  /**
   * Gắn các event listeners cho quiz
   */
  bindEvents() {
    // Next button để chuyển quiz
    document
